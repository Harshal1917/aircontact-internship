...
class HistCandleProvider:
    ...
    def init_candles(self, symbol, start_time, end_time, interval):
        if self.is_running:
            print("Stopping the current session before starting a new one.")
            self.stop_candles()  # Stop the current session if it's active

        # Now proceed to initialize a new session
        self.symbol = symbol
    ...
    #     # Check if target_time is within the fetched date range
    #     if target_time < self.start_time or target_time > self.end_time:
    #         print(f"Target time {target_time} is outside the fetched date range ({self.start_time} to {self.end_time}).")
    #         return "ERROR"  # Target time is outside the fetched date range

    #     print(f"Current queue size: {self.data_queue.qsize()}")
    #     print("Current candles in queue:")
    #     for candle in list(self.data_queue.queue):  # Print all candles in the queue
    #         print(candle)

    #     while not self.data_queue.empty():
    #         candle = self.peek_candle()  # Check the next candle without popping
    #         candle_date = pd.Timestamp(candle['Date']).floor('D')

    #         print(f"Checking candle date: {candle_date} against target date: {target_time}")

    #         if candle_date == target_time:
    #             print(f"Moved to date: {candle_date.strftime('%Y-%m-%d')}")
    #             return "SUCCESS"  # Found a candle on the target date
            
    #         if candle_date > target_time:
    #             print(f"Target date {target_time} not found. No available candle for this date.")
    #             return "ERROR"  # No available candle for the target date
            
    #         self.pop_candle()  # Remove candles before the target date

    #     print("Reached end of queue without finding a suitable candle.")
    #     return "ERROR"  # Reached end of queue without finding a suitable candle
    ...
    def stop_candles(self):
        ...
        # Clear the queue after stopping the candle fetching
        while not self.data_queue.empty():
            self.data_queue.get()  # Remove all candles from the queue
        print("Candle queue has been emptied.")
